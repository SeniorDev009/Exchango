...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['currencies'] = Currency::oldest()->where('status', 1)->get();
        $data['deposits'] = Deposit::where('user_id', auth()->user()->id)->where('status', 0)->latest()->get();
        $data['bitcoin'] =  Currency::find(1);
        $data['democoin'] =  Currency::find(964);
        $data['user_id'] = auth()->user()->id;

        $data['virtual_purchases'] = ShoeTrx::latest()->where('user_id', auth()->user()->id)->get()->groupby(['shoe_id', 'type']);
        // $data['physical_shoes'] = ShoePurchase::latest()->where('user_id', auth()->user()->id)->get()->groupby('status');
        $data['shoes_purchases'] = ShoePurchase::latest()->where('user_id', auth()->user()->id)->paginate(25);
        ...
        // $data['balance'] = Trx::where('user_id', auth()->user()->id )->get()->groupby('currency_id');
    ...
    {
        if (Auth::check() ) {//&& Auth()->user()->email_verify == '1' && Auth()->user()->sms_verify == '1'
...
//            'username' => 'required|min:5||regex:/^\S*$/u|unique:users,username,' . $user->id,
    ...
    public function depositDataInsert(Request $request)
    {
        ...
        $request_amount_in_btc = convertDcoToBitcoin($request->amount);

        /* Limiting Max bitcoins per user to 3 */
        if($request->currency_id == 1){
            $user_bitcoins =    convertDcoToBitcoin(
                                    ( Trx::where('user_id', auth()->user()->id)->where('type', '+')->where('currency_id', $request->currency_id )->sum('amount') 
                                    - Trx::where('user_id', auth()->user()->id)->where('type', '-')->where('currency_id', $request->currency_id )->sum('amount'))
                                );
            if(($user_bitcoins + $request_amount_in_btc ) >= $basic->btc_limit){
                return back()->with('alert', 'Your Bitcoin deposit limit reached!');
            }
            /* Checking exchange limit for bitcoin */
            if( ($currency->available_balance - $request_amount_in_btc) <= 0 ){
                return back()->with('alert', 'Bitcoin exchange deposit limit reached!');
            }
        }
        /* Checking exchange limit for the rest of currencies */
        else if( ($currency->available_balance - $request->amount*$currency->price) <= 0 ){
            return back()->with('alert', 'Currency exchange deposit limit reached!');
        }
        ...
        if ($request->amount <= 0) {
            ...
            $discount = round( (Currency::find($request->currency_id)->discount_percentage / 100) * $request->amount , 8 );
            $request->amount = $request->amount - $discount;

            $discount = $discount + round( Currency::find($request->currency_id)->discount_fixed, 8 );
            $request->amount = $request->amount - Currency::find($request->currency_id)->discount_fixed;
            ...
            if (isset($gate)) {
                if ($gate->minamo <= $request->amount && $gate->maxamo >= $request->amount) {
                    ...
                    $depo['currency_id'] = $request->currency_id;
